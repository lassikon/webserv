...
private:
  ...
  void create(logLevel levelLog, const char *levelTitle, const char *levelColor,
              std::ostream &consoleStream, const char *sourceFile,
              const char *functionName, int lineNumber, Args &&...args) {
...
public:
  template <typename... Args>
  static void Log(logLevel levelLog, const char *levelTitle,
                  const char *levelColor, std::ostream &consoleStream,
                  const char *sourceFile, const char *functionName,
                  int lineNumber, Args &&...args) {
    newLogInstance().create(levelLog, levelTitle, levelColor, consoleStream,
                            sourceFile, functionName, lineNumber, args...);
  }
};

#define LOG_TRACE(...)                                                         \
  (Logger::Log(logLevel::Trace, "TRACE", CYAN, std::cout, __FILE__, __func__,  \
               __LINE__, __VA_ARGS__))
#define LOG_DEBUG(...)                                                         \
  (Logger::Log(logLevel::Debug, "DEBUG", GREEN, std::cout, __FILE__, __func__, \
               __LINE__, __VA_ARGS__))
#define LOG_INFO(...)                                                          \
  (Logger::Log(logLevel::Info, "INFO", BLUE, std::cout, __FILE__, __func__,    \
               __LINE__, __VA_ARGS__))
#define LOG_WARN(...)                                                          \
  (Logger::Log(logLevel::Warn, "WARN", YELLOW, std::cout, __FILE__, __func__,  \
               __LINE__, __VA_ARGS__))
#define LOG_ERROR(...)                                                         \
  (Logger::Log(logLevel::Error, "ERROR", RED, std::cerr, __FILE__, __func__,   \
               __LINE__, __VA_ARGS__))
#define LOG_FATAL(...)                                                         \
  (Logger::Log(logLevel::Fatal, "FATAL", RED, std::cerr, __FILE__, __func__,   \
               __LINE__, __VA_ARGS__))
